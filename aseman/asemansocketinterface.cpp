...
QByteArray AsemanSocketInterface::call(QByteArray data)
{
    ...
    const auto len = p->extraArguments.count();
    for (int i=0; i<len; i++)
        args.insert(i, p->extraArguments.at(i));
...
QVariantList AsemanSocketInterface::extraArguments() const
{
    return p->extraArguments;
}

void AsemanSocketInterface::setExtraArguments(const QVariantList &newExtraArguments)
{
    p->extraArguments = newExtraArguments;
}
